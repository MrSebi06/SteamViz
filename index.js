...
function assignLinksColors(nodes) {
    nodes.forEach(node => {
        const normalizedLinks = normalize(node.links, linksCountRange.min, linksCountRange.max);
...
function setupEventHandlers() {
    ...
    const availableNodes = originalData ? originalData.nodes.length : maxNodeCount;
    ...
    genresButton.addEventListener('click', () => {
        ...
        genreFilter.style.display = currentColorMode === 'genres' ? 'block' : 'none';
    });
