...
def get_top_games(limit=30):
    TOP_GAMES_URL = (
        STEAM_API_URL + "ISteamChartsService/GetGamesByConcurrentPlayers/v1/"
    )
    params = {
        "key": STEAM_API_KEY,
    }
    response = requests.get(TOP_GAMES_URL, params=params)
    response.raise_for_status()
    data = response.json()
    game_ids = [game.get("appid") for game in data.get("response", {}).get("ranks", [])]
...
def get_graph_data():
    ...
    with open("config.json", "w", encoding="utf-8") as file:
        ...
        with open("user_ids.json", "r", encoding="utf-8") as user_ids_file:
            user_ids_data = json.load(user_ids_file)
        ...
        json.dump(
            {
                ...
                "player_accounts_analyzed": len(user_ids),
            },
            file,
            indent=4,
            ensure_ascii=False,
